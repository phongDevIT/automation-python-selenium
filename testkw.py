            ...
            for id, keyword in data.items():
                    ...
                    try: 
                        ...
                        try:
                            ...
                            bid_min_value = bid_min_value.replace("₫", "").strip()
                            bid_max_value = bid_max_value.replace("₫", "").strip()
                            data = {bid_min_value, bid_max_value}
                            
                            # value = [id, keyword, data]
                            value = [keyword, data]
                            print(value, "giá trị key lẻ")
                            # final.append(value)
                            # final[id] = value
                            # final[id] = {keyword : data}
                            final= {
                                'id': id,
                                'keyword': keyword,
                                'data': data
                        ...
                        print(str(final), "kết quả")
            # response = requests.request("POST",'https://click.mmolovers.com/administrator/api/post_keyword', json=json.dumps(final))
